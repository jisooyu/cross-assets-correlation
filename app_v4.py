...
def update_dashboard(n, date_key):
    ...
    # Normalize differently for yields vs cryptos:
    df_norm = df.copy()

    # 1) Yields: index-normalized (start at 1.0)
    yield_cols = [c for c in yields if c in df.columns]
    if yield_cols:
        df_norm[yield_cols] = df[yield_cols] / df[yield_cols].iloc[0]

    # 2) Crypto: z-score normalization so BTC/ETH/XRP movements are visible
    asset_cols = [c for c in assets if c in df.columns]
    if asset_cols:
        df_norm[asset_cols] = (df[asset_cols] - df[asset_cols].mean()) / df[asset_cols].std()
